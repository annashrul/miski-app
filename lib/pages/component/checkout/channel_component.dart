...
class _ChannelComponentState extends State<ChannelComponent> {
  ...
  Future loadData() async {
    final res = await HandleHttp().getProvider(
        "transaction/payment/channel", channelModelFromJson,
        context: context);
    if (res != null) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: WidgetHelper().appBarWithButton(
            context, "Halaman pembayaran", () {}, <Widget>[],
            param: "default"),
        body: isLoading
            ...
            : ListView.separated(
                ...
                itemBuilder: (context, index) {
                  ...
                  return WidgetHelper().titleQ(context, res["name"],
                      subtitle: fee > 0 ? fee : "",
                      fontWeight: FontWeight.normal,
                      fontSize: 9,
                      param: "-",
                      padding: scale.getPadding(0.5, 0), callback: () {
                    checkout(res["code"]);
                  });
                },
              )
